function spG=spgft(PTm,G,rho,Cp,options)
...
%        Alternative is to enter G along a base (reference pressure isobar)-
...
%
%    all remaining parameters are in a structure called "options" and have default values if not provided in the structure
%       "mask" has ones where input thermodynamic properties are defined and NaN where they are not (Default is all ones)
...
%       "algorithm" set to anything to use Least Square fitting (default is Normal equations)
%
